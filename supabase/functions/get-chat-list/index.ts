...
serve(async (req) => {
  ...
  try {
    ...
    // Get rejected rematch requests where current user was the requester
    const { data: rejectedRematchRequests, error: rejectedRematchError } = await supabaseClient
      .from("unmatches")
      .select("*")
      .or(`user1_id.eq.${user.id},user2_id.eq.${user.id}`)
      .eq("rematch_status", "rejected")
      .eq("rematch_requested_by", user.id); // User IS the requester (their request was rejected)

    if (rejectedRematchError) {
      console.error("❌ Error fetching rejected rematch requests:", rejectedRematchError);
    }

    // Add rejected rematch requests to chat list
    if (rejectedRematchRequests && rejectedRematchRequests.length > 0) {
      const rejectedRematchMatches = await Promise.all(
        rejectedRematchRequests.map(async (unmatch) => {
          const otherUserId = unmatch.user1_id === user.id
            ? unmatch.user2_id
            : unmatch.user1_id;
          ...
          // Check if a match already exists (shouldn't happen for rejected, but safety check)
          const user1 = user.id < otherUserId ? user.id : otherUserId;
          const user2 = user.id > otherUserId ? user.id : otherUserId;
          ...
          if (userError || !otherUser) {
            console.error("❌ Error fetching user profile for rejected rematch:", userError);
            return null;
          }
          ...
          return {
            ...
            lastMessageTime: lastMessage?.created_at || unmatch.created_at,
            isUnmatched: true,
            isRematchRejected: true, // Flag to indicate this rematch was rejected
            rematchRequestedBy: unmatch.rematch_requested_by,
          };
        })
      );

      const validRejectedRematchMatches = rejectedRematchMatches.filter((match) => match !== null);
      validMatches.push(...validRejectedRematchMatches);
    }
